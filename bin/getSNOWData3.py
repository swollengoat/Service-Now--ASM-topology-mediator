...
def createAsmRestListenJob(jobName):
   ...
   authHeader = genAuthHeader(snowServerDict["user"], snowServerDict["password"])
   ...
   try:
      request = urllib.request.Request(requestUrl, jsonResource)
...
def genAuthHeader(user, password):

   myUP = user + ":" + password
   userAndPassword = base64.b64encode(myUP.encode()).decode("utf-8")
   header = "Basic " + userAndPassword
   print("returning header: " + header)
   return(header)
...
def getCiData(runType, ciType):
   ...
   if(readFromRest == 1):
      ...
      #authHeader = 'Basic ' + base64.b64encode(snowServerDict["user"].encode("utf-8") + ":" + snowServerDict["password"].encode("utf-8"))
   ...
   for ci in readCiEntries:
      ...
      if "name" in asmObject:
         pass
      else:
         if("ip_address" in asmObject):
            asmObject["name"] = asmObject["ip_address"]
         elif "sys_id" in asmObject:
